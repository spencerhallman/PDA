      ...
      *                                                               *
      * PROGRAM :   PDA102 (IMS MPP PROGRAM)                          *
      * TRANS   :   PDA10201                                          *
      * MFS     :   FORMAT=PDA102, MID=PDA102I, MOD=PDA102O           *
      *                                                               *
      * FUNCTION:   PROGRAM PDA102 IS THE IMS/DC PRODUCT DEMONSTRATION*
      ...
      *                                                               *
      *             ADD ORDER PROCESS                                 *
      *             ORDER INQUIRY / MAINTENANCE                       *
      *             BROWSE SUBMITTED ORDERS INQUIRY / SELECTION       *
      *             PENDING ORDER INQUIRY / MAINTENANCE               *
      *                                                               *
      *                                                               *
      * FILES   :   NONE                                              *
      *                                                               *
      ...
      *             PDA11001   ORDER INQUIRY / MAINTENANCE            *
      *             PDA11201   BROWSE SUBMITTED ORDERS                *
      *             PDA10101   MAIN MENU                              *
      *                                                               *
      *                                                               *
      * PFKEYS  :   PF12 - RETURN TO MAIN MENU                        *
      *                                                               *
      *****************************************************************
      *             PROGRAM CHANGE LOG                                *
      *             -------------------                               *
      ...
      *****************************************************************
       01  WS-MISCELLANEOUS-FIELDS.
           ...
           05  WMF-MODNAME             PIC X(08)   VALUE 'PDA102O'.
           ...
           05  WMF-ORDER-ADD-MESSAGE.
               10 WMF-ORDER-PROCESSED  PIC X(24).
               10 WMF-ORDER-NBR-TEXT   PIC X(18).
               10 WMF-ORDER-NBR        PIC X(10).
      ...
      *****************************************************************
      *  PDA ORDER MENU INPUT / OUTPUT MESSAGE STORAGE AREA           *
      *  PREFIX: PDA102                                               *
      *****************************************************************

       01  PDA102-MESSAGE              REDEFINES CIOM-MESSAGE.
           05 PDA102-MSG-LL            PIC S9(04)      COMP.
           05 PDA102-MSG-ZZ            PIC X(02).
           05 PDA102-MSG-TRANCODE      PIC X(08).
           05 FILLER                   PIC X(01).
           05 PDA102-MSG-SOURCE        PIC X(03).
           05 FILLER                   PIC X(01).
           05 PDA102-PFKEY             PIC X(02).
           05 PDA102-MSG-USERID-INFO.
              10  PDA102-USERID-ID     PIC X(08).
              10  PDA102-USERID-NUMBER PIC 9(05).
           05 PDA102-PREV-PGRMID       PIC X(08).
           05 PDA102-SAVAREA           PIC X(79).
           05 PDA102-SAVAREA-R         REDEFINES PDA102-SAVAREA.
              10  PDA102-SAVAREA-ORDER-MENU-SEL
                                       PIC X(01).
              10  PDA102-SAVAREA-CUSID PIC X(32).
              10  FILLER               PIC X(10).
              10  PDA102-SAVAREA-MESSAGE
                                       PIC X(05).
              10  PDA102-SAVAREA-ORDER PIC X(10).
           05 PDA102-MENU-SELECTION-ATTR.
              10  PDA102-MENU-SELECTION-ATTR1
                                       PIC X(01).
              10  PDA102-MENU-SELECTION-ATTR2
                                       PIC X(01).
           05 PDA102-MENU-SELECTION    PIC X(01).
           05 PDA102-SCREEN-MESSAGE    PIC X(79).
           05 PDA102-SMESSAGE          PIC X(79).
           EJECT
      ...
      *****************************************************************
           ...
           ELSE
               ...
               MOVE 'PDA102'           TO WPIE-PROGRAM-ID
      ...
      *****************************************************************
           ...
           IF CIOM-PREV-PGRMID     NOT = 'PDA102'
      ...
      *****************************************************************
           ...
           IF CIOM-MSG-TRANCODE        = 'PDA10201'    AND
              CIOM-MSG-SOURCE          = 'PDA'
               NEXT SENTENCE
           ELSE
               MOVE 'IMS'              TO WS-PDA-ERROR-TYPE
      ...
      *****************************************************************
      *    IF 1ST TIME INQUIRY TRANS CHECK FOR MESSAGES FROM OTHER    *
      *    PROGRAMS,  EXIT ROUTINE TO DISPLAY THE MENU                *
      *****************************************************************

           IF INQUIRY-TRANS
               PERFORM  P04000-CHECK-MESSAGES
                   THRU P04000-CHECK-MESSAGES-EXIT
               GO TO P03000-EDIT-PROCESS-EXIT.


      *****************************************************************
      *    TRANSFER TO NEXT PROGRAM BASED ON PFKEY / MENU SELECTION   *
      *****************************************************************
      ...
      *    ON THE PFKEY / MENU SELECTION                              *
      *                                                               *
      *    TRANSACTION PDA10101 = MAIN MENU (VIA PFKEY 12)            *
      *    TRANSACTION PDA10401 = CUSTOMER IDENTIFICATION             *
      *    TRANSACTION PDA11001 = ORDER / INQUIRY MAINTENANCE         *
      *    TRANSACTION PDA11201 = BROWSE SUBMITTED ORDERS             *
      *****************************************************************

      *****************************************************************
      *    IF PFKEY USAGE, SET TRANSACTION ID, EXIT                   *
      *****************************************************************
           IF PDA102-PFKEY  =  '12'
      ...
      *****************************************************************
      *    OTHERWISE SET TRANSACTION ID BASED ON MENU OPTION SELECTED *
      *****************************************************************
           IF SELECTION-IS-ADD-ORDER       OR
              SELECTION-IS-PENDING-ORDER
      ...
      *****************************************************************
           ...
           IF PDA102-PFKEY  =  'EN'  OR  '12'
               NEXT SENTENCE
           ELSE
               MOVE LOW-VALUES         TO  PDA102-MENU-SELECTION-ATTR
               MOVE WS-CURSOR-ATTR     TO  PDA102-MENU-SELECTION-ATTR1
      ...
      *****************************************************************
           ...
           IF (PDA102-PFKEY      NOT = 'EN')   AND
              (PDA102-MENU-SELECTION > SPACES)
               MOVE LOW-VALUES         TO  PDA102-MENU-SELECTION-ATTR
               MOVE WS-CURSOR-ATTR     TO  PDA102-MENU-SELECTION-ATTR1
      ...
      *****************************************************************
      *    MENU SELECTION MUST BE NUMERIC, AND VALUE OF 1,2,3, OR 4   *
      *****************************************************************

           MOVE PDA102-MENU-SELECTION  TO WS-MENU-SELECTION-SW.

           IF SELECTION-IS-VALID
               MOVE PDA102-MENU-SELECTION
                                       TO PDA102-SAVAREA-ORDER-MENU-SEL
           ELSE
               MOVE LOW-VALUES         TO  PDA102-MENU-SELECTION-ATTR
               MOVE WS-CURSOR-ATTR     TO  PDA102-MENU-SELECTION-ATTR1
               MOVE WS-HI-INTENSITY-ATTR
                                       TO  PDA102-MENU-SELECTION-ATTR2
      ...
      *****************************************************************
      *                                                               *
      *    PARAGRAPH:  P04000-CHECK-MESSAGES                          *
      *                                                               *
      *    FUNCTION :  ROUTINE TO CHECK FOR MESSAGES RETURNED FROM    *
      *                OTHER MODULES IN THE IMS MESSAGE AREA          *
      *                PDA102-SAVAREA AND DISPLAYED ON THE MENU       *
      *                SCREEN                                         *
      *                                                               *
      *    CALLED BY:  P03000-EDIT-PROCESS                            *
      *                                                               *
      *****************************************************************
       ...
       P04000-CHECK-MESSAGES.

           IF PDA102-SAVAREA-MESSAGE   =  'PM029'
               MOVE PM029-ORDER-PROCESSED
                                       TO WMF-ORDER-PROCESSED
               MOVE ', ORDER NUMBER IS '
                                       TO WMF-ORDER-NBR-TEXT
               MOVE PDA102-SAVAREA-ORDER
                                       TO WMF-ORDER-NBR
               MOVE WMF-ORDER-ADD-MESSAGE
                                       TO PDA102-SCREEN-MESSAGE
               MOVE SPACES             TO PDA102-SAVAREA-MESSAGE
                                          PDA102-SAVAREA-ORDER
           ELSE

           IF PDA102-SAVAREA-MESSAGE   = 'PM030'
               MOVE PM030-ORDER-CANCELLED
                                       TO PDA102-SCREEN-MESSAGE
               MOVE SPACES             TO PDA102-SAVAREA-MESSAGE
           ELSE
               NEXT SENTENCE.
      ...
      *****************************************************************
       ...
       P70000-ERROR-ROUTINE.
           ...
           IF PDA102-SCREEN-MESSAGE    >  SPACES
               NEXT SENTENCE
           ELSE
               MOVE WMF-MESSAGE-AREA   TO PDA102-SCREEN-MESSAGE.
      ...
      *****************************************************************
           ...
           MOVE 'PDA102'               TO PDA102-PREV-PGRMID.

           INSPECT PDA102-MENU-SELECTION
               CONVERTING  WMF-SPACES-LOWVALUE-R TO '__'.

           MOVE LENGTH OF PDA102-MESSAGE
                                       TO PDA102-MSG-LL.
      ...
      *****************************************************************
           ...
           MOVE 'PDA102'               TO CIOM-PREV-PGRMID.
