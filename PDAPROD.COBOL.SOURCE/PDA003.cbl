      ...
      * MAPSET  :   PDA003M                                           *
      *                                                               *
      * FUNCTION:   PROGRAM PDA003 IS THE PRODUCT DEMONSTRATION       *
      *             APPLICATION MAINTENANCE MENU PROGRAM. IT WILL     *
      *             CONTAIN THE FOLLOWING SELECTIONS:                 *
      *                                                               *
      *                 BASE DATA REFRESH                             *
      ...
      *****************************************************************
       01  WS-SWITCHES.
           ...
           05  WS-MENU-SELECTION-SW    PIC X(01)             VALUE ' '.
               ...
               88  SELECTION-IS-SCENARIO-SELECT              VALUE '9'.
      ...
      *****************************************************************
       ...
       01  WS-PDA-COMMAREA.
           COPY PDACOMM.
           05  FILLER                  REDEFINES PC-PROGRAM-WORKAREA.
               07  PC-CONFIRM          PIC X.
           EJECT
      ...
      *****************************************************************
           ...
           IF EIBCALEN                 > ZEROES
               ...
               IF EIBCALEN             NOT = PC-COMMAREA-LTH
                   ...
                   MOVE 'PDA003'       TO WPCE-PROGRAM-ID
      ...
      *****************************************************************
           ...
           MOVE 'PDA003'               TO PC-PREV-PGRMID.
           MOVE LOW-VALUES             TO PDA003I.
      ...
      *****************************************************************
           ...
           IF EIBAID  = DFHENTER  OR  DFHPF12  OR DFHCLEAR
               NEXT SENTENCE
           ELSE
               MOVE -1                 TO  MENUSELL
      ...
      *****************************************************************
      *    MENU SELECTION MUST BE NUMERIC, AND VALUE OF 8 OR 9        *
      *****************************************************************
      ...
      *****************************************************************
      *                                                               *
      *    PARAGRAPH:  P04000-USERID-UTIL                             *
      *                                                               *
      *    FUNCTION :  ROUTINE TO GET UNIQUE NUMERIC ID FOR SIGN-ON ID*
      *                                                               *
      *    CALLED BY:  P03100-EDIT-SCREEN                             *
      *                                                               *
      *****************************************************************
      ...
      *****************************************************************
           ...
           PERFORM P06000-SELECT-FROM-USERID
              THRU P06000-SELECT-FROM-USERID-EXIT.

           MOVE SPACES                 TO MENUSELI.
           MOVE -1                     TO MENUSELL.
           MOVE USERID-NUMBER          TO PM006-MSG-IDNUM
           MOVE PM006-NUMBER-FOR-USERID
                                       TO WMF-MESSAGE-AREA
           MOVE WMF-MESSAGE-AREA       TO PDAMSGO.

       P04000-USERID-UTIL-EXIT.
           EXIT.
           EJECT

      *****************************************************************
      *                                                               *
      *    PARAGRAPH:  P06000-SELECT-FROM-USERID                      *
      *                                                               *
      *    FUNCTION :  ROUTINE TO ACCESS DB2 USERID TABLE TO OBTAIN   *
      *                UNIQUE ID                                      *
      *                                                               *
      *    CALLED BY:  P04000                                         *
      *                                                               *
      *****************************************************************
       ...
       P06000-SELECT-FROM-USERID.

           EXEC SQL
             SELECT NUMBER INTO :USERID-NUMBER
               FROM USERID
                    WHERE ID = :USERID-ID
           END-EXEC.
           ...
           IF SQLCODE NOT = ZERO
             ...
             MOVE 'PDA003'             TO WPDE-PROGRAM-ID
             MOVE SQLCODE              TO WPDE-DB2-SQLCODE
             MOVE 'SELECT FROM USERID' TO WPDE-FUNCTION
             MOVE 'P04000'             TO WPDE-PARAGRAPH
             PERFORM P99500-PDA-ERROR
                THRU P99500-PDA-ERROR-EXIT.

       P06000-SELECT-FROM-USERID-EXIT.
           EXIT.
           EJECT
...
AS OF      MOVE 'PDA003'               TO WPCE-PROGRAM-ID.               AS OF
